...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        head_outputs = [h(x) for h in self.heads]  # List of head outputs
        out = torch.cat(head_outputs, dim=-1)  # Concatenate along the last dimension
...
class MultiHeadAttentionModel(nn.Module):
    def __init__(self, output_size, embeddings, max_length=60, n_head=3):
        ...
        head_size = dim // self.n_head
        # def __init__(self, num_heads, head_size, n_embd, dropout=0.0):
        self.sa = MultiHeadAttention(self.n_head, head_size, dim)  # Fix head_size argument
        self.ffwd = FeedFoward(dim)
        self.ln1 = nn.LayerNorm(dim)
        self.ln2 = nn.LayerNorm(dim)

        self.output = nn.Linear(dim, output_size)  # Output classification layer
    ...
    def forward(self, x, lengths):
        ...
        x = torch.mean(x, dim=1)  # Avg pooling to get a sentence embedding (B,C)
