...
def train_dataset(_epoch, dataloader, model, loss_function, optimizer):
    ...
    for index, batch in enumerate(dataloader, 1):
        ...
        y_hat = model(inputs,lengths)  # EX9

        if DATASET == "MR":
            labels_hat = torch.FloatTensor([[1,0] if i == 0 else [0,1] for i in labels])
        else:
            labels_hat = labels
        
        # Step 3 - compute loss: L = loss_function(y, y')
        loss = loss_function(y_hat, labels_hat)  # EX9
...
def eval_dataset(dataloader, model, loss_function):
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader, 1):
            ...
            y_preds = torch.argmax(y_hat, dim=1)  # EX9
